...
df['City Rank'] = [city_rank[x] for x in df['City']]

state_rank = {}
one_hot_encode(state_rank, df['State'])
df['State Rank'] = [state_rank[x] for x in df['State']]

vin_rank = {}
one_hot_encode(vin_rank, df['Vin'])
df['Vin Rank'] = [vin_rank[x] for x in df['Vin']]

make_rank = {}
one_hot_encode(make_rank, df['Make'])
df['Make Rank'] = [make_rank[x] for x in df['Make']]

model_rank = {}
one_hot_encode(model_rank, df['Model'])
df['Model Rank'] = [model_rank[x] for x in df['Model']]
...
w = [((random.random() * 0.0001)) for x in range(len(columns) + 1)]
print('W:')
print(w)
...
# df = pd.read_csv('train.csv').iloc[1100:1110, :]
#
